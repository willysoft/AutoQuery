...
public class QueryExtensionsCursorPaginationTests
{
    ...
    [Fact]
    public void ApplyQueryCursorPaged_SortByDifferentField_ShouldIncludeCursorKeyInSort()
    {
        // Arrange
        var queryProcessor = new QueryProcessor();
        var builder = new FilterQueryBuilder<TestCursorQueryOptions, TestData>();
        ...
        var testData = new List<TestData>
        {
            new TestData { Id = 3, Name = "Alice" },
            new TestData { Id = 1, Name = "Bob" },
            new TestData { Id = 4, Name = "Charlie" },
            new TestData { Id = 2, Name = "Diana" },
            new TestData { Id = 5, Name = "Eve" },
        }.AsQueryable();

        // Act - First page sorted by Name (cursor key Id will be PRIMARY sort, Name secondary)
        // Effective sort becomes: id,name
        var firstPageOptions = new TestCursorQueryOptions { PageSize = 2, Sort = "name" };
        var firstPage = testData.ApplyQueryCursorPaged(queryProcessor, firstPageOptions);

        // Assert - First page should have id=1 (Bob) and id=2 (Diana) - sorted by Id first
        Assert.Equal(2, firstPage.Count);
        Assert.Equal(1, firstPage.Datas.First().Id);
        Assert.Equal(2, firstPage.Datas.Last().Id);
        Assert.NotNull(firstPage.NextPageToken);
        ...
        // Assert - Second page should have id=3 and id=4
        Assert.Equal(2, secondPage.Count);
        Assert.Equal(3, secondPage.Datas.First().Id);
        Assert.Equal(4, secondPage.Datas.Last().Id);
        Assert.NotNull(secondPage.NextPageToken);
        ...
        // Assert - Third page should have id=5 only
        Assert.Equal(1, thirdPage.Count);
        Assert.Equal(5, thirdPage.Datas.First().Id);
        Assert.Null(thirdPage.NextPageToken);
    }

    [Fact]
    public void ApplyQueryCursorPaged_SortByDifferentFieldDescending_ShouldIncludeCursorKeyInSort()
    {
        // Arrange
        var queryProcessor = new QueryProcessor();
        var builder = new FilterQueryBuilder<TestCursorQueryOptions, TestData>();
        ...
        // Assert - First page should have id=1 and id=2 - sorted by Id first
        Assert.Equal(2, firstPage.Count);
        Assert.Equal(1, firstPage.Datas.First().Id);
        Assert.Equal(2, firstPage.Datas.Last().Id);
        Assert.NotNull(firstPage.NextPageToken);
    ...
    [Fact]
    public void ApplyQueryCursorPaged_NoSort_ShouldUseCursorKeyAsDefaultSort()
    {
        // Arrange
        var queryProcessor = new QueryProcessor();
        var builder = new FilterQueryBuilder<TestCursorQueryOptions, TestData>();
        ...
        // Act - No sort specified
        var firstPageOptions = new TestCursorQueryOptions { PageSize = 2 };
        var firstPage = testData.ApplyQueryCursorPaged(queryProcessor, firstPageOptions);

        // Assert - Should be sorted by Id (cursor key)
        Assert.Equal(2, firstPage.Count);
        Assert.Equal(1, firstPage.Datas.First().Id);
        Assert.Equal(2, firstPage.Datas.Last().Id);
        Assert.NotNull(firstPage.NextPageToken);
